...
async function testErrorClasses(results) {
  ...
  await runTest(results, 'errorClasses', 'ValidationError creation and properties', () => {
    ...
    if (error.name !== 'ValidationError') throw new Error('Wrong error name');
    if (error.code !== 'VALIDATION_ERROR') throw new Error('Wrong error code');
    if (error.field !== 'testField') throw new Error('Wrong field');
    if (error.value !== 'badValue') throw new Error('Wrong value');
    if (error.expectedType !== 'string') throw new Error('Wrong expected type');
  ...
  await runTest(results, 'errorClasses', 'SwarmError with context', () => {
    ...
    if (error.name !== 'SwarmError') throw new Error('Wrong error name');
    if (error.swarmId !== 'test-swarm-id') throw new Error('Wrong swarm ID');
    if (error.operation !== 'initialization') throw new Error('Wrong operation');
  ...
  await runTest(results, 'errorClasses', 'ErrorContext management', () => {
    ...
    if (context.get('operation') !== 'test') throw new Error('Context get failed');
...
async function testValidationSystem(results) {
  ...
  await runTest(results, 'validation', 'Parameter validation with valid inputs', () => {
    ...
    if (result.topology !== 'mesh') throw new Error('Topology validation failed');
    if (result.maxAgents !== 10) throw new Error('MaxAgents validation failed');
    if (result.strategy !== 'balanced') throw new Error('Strategy validation failed');
  });
  ...
  await runTest(results, 'validation', 'Parameter validation with defaults', () => {
    ...
    if (result.topology !== 'mesh') throw new Error('Default topology not applied');
    if (result.maxAgents !== 5) throw new Error('Default maxAgents not applied');
    if (result.strategy !== 'balanced') throw new Error('Default strategy not applied');
  });
  ...
  await runTest(results, 'validation', 'Parameter validation with invalid inputs', () => {
    ...
    if (!errorCaught) throw new Error('Expected validation to fail');
  });
  ...
  await runTest(results, 'validation', 'Schema documentation generation', () => {
    ...
    if (!doc || !doc.parameters) throw new Error('Schema doc missing');
    if (!doc.parameters.topology) throw new Error('Topology parameter missing');
    if (!doc.parameters.topology.allowedValues) throw new Error('Allowed values missing');
    
    const allowedValues = doc.parameters.topology.allowedValues;
    if (!allowedValues.includes('mesh')) throw new Error('Expected allowed values missing');
  });
}
...
async function testMCPIntegration(results) {
  ...
  await runTest(results, 'integration', 'MCP Tools initialization', async () => {
    ...
    if (!tools.errorLog) throw new Error('Error log not initialized');
    if (!Array.isArray(tools.errorLog)) throw new Error('Error log not array');
  });
  ...
  await runTest(results, 'integration', 'swarm_init error handling', async () => {
    ...
    if (!errorCaught) throw new Error('Expected swarm_init to throw error');
  ...
  await runTest(results, 'integration', 'Error statistics tracking', async () => {
    ...
    if (stats.total !== 2) throw new Error('Wrong total error count');
    if (stats.bySeverity.medium !== 1) throw new Error('Wrong medium severity count');
    if (stats.bySeverity.high !== 1) throw new Error('Wrong high severity count');
    if (stats.byTool.swarm_init !== 1) throw new Error('Wrong tool count');
  });
}
...
async function testPerformanceAndEdgeCases(results) {
  ...
  await runTest(results, 'performance', 'Empty parameters handling', () => {
    ...
    if (!result.topology) throw new Error('Default not applied for empty params');
  });
  ...
  await runTest(results, 'performance', 'Null parameters handling', () => {
    ...
    if (!result.topology) throw new Error('Default not applied for null params');
  });

...
pub trait DynamicResourceManager: Send + Sync {
    ...
    #[cfg(feature = "async")]
    async fn allocate_resources(&mut self, request: &ResourceRequest) -> Result<ResourceAllocation>;
...
pub trait PerformanceOptimizer: Send + Sync {
    ...
    #[cfg(feature = "async")]
    async fn tune_performance(&mut self, recommendations: &[TuningRecommendation]) -> Result<TuningResult>;
    
    /// Tune performance (sync version)
    #[cfg(not(feature = "async"))]
    fn tune_performance(&mut self, recommendations: &[TuningRecommendation]) -> Result<TuningResult>;
...
pub trait MemoryManager: Send + Sync {
    /// Allocate memory
    #[cfg(feature = "async")]
    async fn allocate_memory(&mut self, size: usize, priority: MemoryPriority) -> Result<MemoryAllocation>;
    
    /// Allocate memory (sync version)
    #[cfg(not(feature = "async"))]
    fn allocate_memory(&mut self, size: usize, priority: MemoryPriority) -> Result<MemoryAllocation>;
...
mod tests {
    ...
    fn test_memory_statistics() {
        ...
        assert_eq!(stats.allocated_memory + stats.free_memory, stats.total_memory);
    }
}
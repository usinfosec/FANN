...
#[cfg(feature = "wasm")]
use js_sys::{Array, Object, Promise, Function};
...
impl WasmResourceManager {
    ...
    pub fn new(max_memory_mb: u32) -> Self {
        console::log_1(&format!("Creating WASM resource manager with {}MB limit", max_memory_mb).into());
    ...
    pub fn allocate_memory(&mut self, size_mb: u32) -> bool {
        if self.allocated_memory + size_mb <= self.max_memory {
            ...
            console::log_1(&format!("Allocated {}MB, total: {}MB", size_mb, self.allocated_memory).into());
            true
        } else {
    ...
    pub fn deallocate_memory(&mut self, size_mb: u32) -> bool {
        if size_mb <= self.allocated_memory {
            self.allocated_memory -= size_mb;
            console::log_1(&format!("Deallocated {}MB, total: {}MB", size_mb, self.allocated_memory).into());
            true
        } else {

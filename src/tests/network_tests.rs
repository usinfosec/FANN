use crate::{Network, Layer, Neuron, Connection, NetworkBuilder, ActivationFunction};
...
fn test_network_builder_with_activation_functions() {
    ...
    for neuron in &hidden_layer.neurons[..3] { // Exclude bias neuron
        assert_eq!(neuron.activation_function, ActivationFunction::ReLU);
    }
    
    let output_layer = &network.layers[2];
    assert_eq!(output_layer.neurons[0].activation_function, ActivationFunction::Linear);
}
...
fn test_network_fully_connected() {
    ...
    let hidden_layer = &network.layers[1];
    for i in 0..3 { // For each hidden neuron (excluding bias)

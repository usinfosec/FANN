...
impl<T: Float> StepDecay<T> {
    pub fn new(initial_rate: T, drop_rate: T, epochs_per_drop: usize) -> Self {
        Self { initial_rate, drop_rate, epochs_per_drop }
    }
}
...
pub trait StopCriteria<T: Float> {
    fn should_stop(&self, trainer: &dyn TrainingAlgorithm<T>, network: &Network<T>, data: &TrainingData<T>, epoch: usize) -> bool;
}
...
impl<T: Float> StopCriteria<T> for MseStopCriteria<T> {
    fn should_stop(&self, trainer: &dyn TrainingAlgorithm<T>, network: &Network<T>, data: &TrainingData<T>, _epoch: usize) -> bool {
...
pub use backprop::{IncrementalBackprop, BatchBackprop};
pub use rprop::Rprop;
...
pub(crate) mod helpers {
            ...
            for neuron in &mut current_layer.neurons {
                if !neuron.is_bias {
                    ...
                    if !neuron.connections.is_empty() {
                        neuron.connections[0].weight = neuron.connections[0].weight + bias_updates[weight_layer_idx][neuron_idx];
                    }
                    
                    // Update weights (skip bias connection)
                    for connection in neuron.connections.iter_mut().skip(1) {
                        connection.weight = connection.weight + weight_updates[weight_layer_idx][weight_idx];
                        weight_idx += 1;
                    }
    ...
    pub fn calculate_gradients<T: Float>(
        ...
        let mut weight_gradients = network.weights.iter()
            .map(|w| vec![T::zero(); w.len()])
            .collect::<Vec<_>>();
        let mut bias_gradients = network.biases.iter()
            ...
            for neuron_idx in 0..network.layer_sizes[layer_idx] {
                ...
                for next_neuron_idx in 0..network.layer_sizes[layer_idx + 1] {
                    ...
                    if weight_idx < network.weights[layer_idx].len() {
                        error_sum = error_sum + errors[0][next_neuron_idx] * network.weights[layer_idx][weight_idx];
                    }
                }
                
                layer_errors[neuron_idx] = error_sum * sigmoid_derivative(activations[layer_idx][neuron_idx]);
            }

...
impl<T: Float + Send> Quickprop<T> {
    ...
    fn initialize_state(&mut self, network: &Network<T>) {
        if self.previous_weight_gradients.is_empty() {
            ...
            self.previous_weight_gradients = network.layers.iter()
                .skip(1) // Skip input layer
                .map(|layer| {
                    let num_neurons = layer.neurons.len();
                    let num_connections = if layer.neurons.is_empty() { 0 } else { layer.neurons[0].connections.len() };
                    vec![T::zero(); num_neurons * num_connections]
                })
                .collect();
                
            self.previous_bias_gradients = network.layers.iter()
                .skip(1) // Skip input layer
                .map(|layer| vec![T::zero(); layer.neurons.len()])
                .collect();
                
            self.previous_weight_deltas = network.layers.iter()
                .skip(1) // Skip input layer
                .map(|layer| {
                    let num_neurons = layer.neurons.len();
                    let num_connections = if layer.neurons.is_empty() { 0 } else { layer.neurons[0].connections.len() };
            ...
            self.previous_bias_deltas = network.layers.iter()
                .skip(1) // Skip input layer
                .map(|layer| vec![T::zero(); layer.neurons.len()])
                .collect();
        }
    ...
    fn calculate_quickprop_delta(
        ...
        if delta.abs() > max_delta {
            delta = if delta > T::zero() { max_delta } else { -max_delta };
        }
...
impl<T: Float + Send> TrainingAlgorithm<T> for Quickprop<T> {
    ...
    fn save_state(&self) -> TrainingState<T> {
        ...
        state.insert("previous_weight_gradients".to_string(), all_weight_gradients);
